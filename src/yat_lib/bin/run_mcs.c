	...
	if (cycles_ms) {
		...
		while (now + last_loop < start + exec_time) {
			...
			if (emergency_exit && wctime() > emergency_exit) {
				 ...
				 * execution time tracking is broken in the YAT^RT
...
static int generate_output(int output_fd)
{
	...
	len = snprintf(buf, 4095, "(rtspin/%d:%u completed: %s @ %" PRIu64 "ns)\n",
			getpid(), job_no, input_buf, (uint64_t) yat_clock());
...
static void job(double exec_time, double program_end, int resource_num, int* lock_od, int * num_of_access, double* cs_length)
{
	...
	if (resource_num > 0) {
        ...
        for(i=0;i<resource_num;i++){
            sum_length += num_of_access[i]*cs_length[i];
        }
        chunk1 = 0;
        for(i=0;i<resource_num;i++){
            /* simulate critical section somewhere in the middle */
            double non_critical_time = drand48()*(exec_time - sum_length)/resource_num;
            chunk1 += non_critical_time;

            /* non-critical section */
            loop_for(non_critical_time, program_end + 1);
            for(j=0;j<num_of_access[i];j++){
                /* critical section */
                yat_lock(lock_od[i]);
                loop_for(cs_length[i], program_end + 1);
                printf("enter critical lock_od:%d\n",lock_od[i]);
                yat_unlock(lock_od[i]);
            }
        }
        /* non-critical section */
        chunk2 = exec_time - sum_length - chunk1;
...
int main(int argc, char** argv)
{
	...
	unsigned int priority = YAT_NO_PRIORITY;
	int migrate = 0;
	int cluster = 0;
	int reservation = -1;
    ...
    int resource_num=0;
    int* num_of_access=NULL;
	const char *lock_namespace = "./rtspin-locks";
	int protocol = -1;
	double* cs_length = NULL; /* millisecond */
	...
	while ((opt = getopt(argc, argv, OPTSTR)) != -1) {
		...
		/*case 'L':
			...
			break;*/
		case 'Q':
			resource_num = want_non_negative_int(optarg, "-Q");
	...
	if (protocol >= 0 && resource_num>0) {
        /*initialize*/
        lock_od=(int *)malloc(resource_num*sizeof(int));
        resource_id=(int *)malloc(resource_num*sizeof(int));
        cs_length=(double *)malloc(resource_num*sizeof(double));
        num_of_access=(int *)malloc(resource_num*sizeof(int));
        for(i=0;i<resource_num;i++){
            num_of_access[i]=resource_id[i]=cs_length[i]=0;
        }
        for(i=0;i<resource_num;i++){
            if(optind + 3 + 3*i <=argc)
                resource_id[i] = want_positive_double(argv[optind + 3 + 3*i ], "RESOURCE_ID");
            if(optind + 3 + 3*i + 1<=argc)
                num_of_access[i] = want_positive_double(argv[optind + 3 + 3*i + 1], "NUM_OF_ACCESS");
            if(optind + 3 + 3*i + 2<=argc)
                cs_length[i] = want_positive_double(argv[optind + 3 + 3*i + 2], "CS_LENGTH")*0.001;
        }
		/* open reference to semaphore */
        for(i=0;i<resource_num;i++){
            lock_od[i] = yat_open_lock(protocol, resource_id[i], lock_namespace, &cluster);
            if (lock_od[i] < 0) {
                perror("yat_open_lock");
	...
	next_release = cp ? cp->release : yat_clock();
	...
	while (1) {
		...
		/* burn cycles */
		job(acet, start + duration, resource_num, lock_od, num_of_access, cs_length);
		...
		if (!sporadic) {
			...
			if (linux_sleep) {
				...
				/* Use YAT^RT API: some plugins optimize
