...
static void job(int thread_id, double program_end, int lock_od,double cs_length)
{
	int i=0;
	switch(thread_id){
		case 1:
		...
		case 7:
			...
			break;
		default:
			printf("thread id %d,excute default task\n",thread_id);
		break;
	}
}
...
#define OPTSTR "p:c:wlveo:s:m:q:r:X:L:Q:iRu:U:Bhd:C:S::O::TD:E:A:a:t:"
...
int main(int argc, char** argv)
{
	...
	while ((opt = getopt(argc, argv, OPTSTR)) != -1) {
		...
		case 'Q':
			...
			break;
        case 't':
            thread_id=want_non_negative_int(optarg, "-t");
            if (thread_id < 0)
				usage("Invalid thread id.");
			break;
		case 'v':
			verbose = 1;
			break;
	...
	if (protocol >= 0) {
        ...
        if(resource_id==0)
        	resource_id=1;
		lock_od = yat_open_lock(protocol, resource_id, lock_namespace, &cluster);
		if (lock_od < 0) {
	...
	while (1) {
		...
		/* burn cycles */
		job(thread_id,start + duration, lock_od,cs_length*0.001);
