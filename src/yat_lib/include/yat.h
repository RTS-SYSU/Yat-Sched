/**
 * @file yat.h
 * Public API for YAT^RT
 */

/**
 * @mainpage
 * The YAT^RT patch is a (soft) real-time extension of the Linux kernel with a
 ...
 *
 * \b yat_lib is the userspace API for YAT^RT. It consists of functions to
...
/**
 * @private
 * The numeric ID of the YAT^RT scheduling class.
 */
#define SCHED_YAT 7
 ...
 */
int yat_open_lock(obj_type_t protocol, int lock_id, const char* name_space,
		void *config_param);

/**
 * Obtain lock
 * @param od Object descriptor obtained by yat_open_lock()
 ...
 * @todo Document
 */
int read_yat_stats(int *ready, int *total);
...
/** Get the current time used by the YAT^RT scheduler.
